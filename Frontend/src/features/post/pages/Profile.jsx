...
const Profile = () => {
  ...
  const { feed, handleGetFeed, handleDeletePost } = usePost();
  ...
  useEffect(() => {
    if (feed && user) {
      const posts = feed.filter((post) => post.user?._id === user.id);
      setUserPosts(posts);
    }
  }, [feed, user]);
  ...
  const handleSaveDp = async () => {
    try {
      // Only update if there's a selected file or if bio has changed
      if (selectedFile || dpUrl !== user?.profileImage) {
        console.log(
          "Updating profile with file:",
          selectedFile?.name || "existing image",
        );
        await handleUpdateProfile(user.bio, selectedFile);
    ...
    <main className="profile-page">
      ...
      <div className="profile-container">
        <div className="profile-header">
          <div className="profile-avatar" onClick={openDpModal}>
            <img src={toPublicUrl(user.profileImage)} alt="Profile" />
